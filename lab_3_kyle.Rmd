...
```{r}
fixed_effects = plm(total_fatal_per_pop ~ dum_year + 
                           dum_blood_alc +
                           no_proof_guilt +
                           seat_belt +
                           speed_limit_above_70 +
...
1. Independence across Units and Time: The error term should be uncorrelated with the explanatory variables, and the observations should be independent across individuals and time.
2. No Endogeneity: The individual-specific effect (unobserved heterogeneity) should be uncorrelated with the independent variables.
3. Homoskedasticity: The error term should have constant variance.
4. No Serial Correlation: There should be no correlation between the error term of different time periods for the same individual.
...
apr_2020_2018_diff = apr_2020 - apr_2018
raw_percent_apr_2020_2018 = apr_2020 / apr_2018 - 1

jan_2020_2018_diff = jan_2020 - jan_2018
raw_percent_jan_2020_2018 = jan_2020 / jan_2018 - 1
```
...
miles_per_capita_apr_2020 = apr_2020/p_2020

miles_per_capita_apr_2018 = apr_2018/p_2018
diff_per_capita_apr = apr_2020/329.5 - apr_2018/326.8
change_100000_apr = 0.00093498*(apr_2020/329.5 - apr_2018/326.8)*100000
change_apr_percent = (apr_2020/329.5 - apr_2018/326.8)/(apr_2018/326.8)
```
```{r message = FALSE, echo =FALSE, warning = FALSE}

miles_per_capita_jan_2020 = jan_2020/p_2020

miles_per_capita_jan_2018 = jan_2018/p_2018
diff_per_capita_jan = jan_2020/329.5-jan_2018/326.8

change_100000_jan =0.00093498*(jan_2020/329.5-jan_2018/326.8)*100000
change_jan_percent = (jan_2020/329.5-jan_2018/326.8)/(jan_2018/326.8)
```
```{r echo = FALSE, message = FALSE}
summary(my_panel$total_fatal_per_pop)

```
The month of April 2020 demonstrated the largest decrease in driving, from `r apr_2018` in April 2018 to `r apr_2020` in April 2020. This was a 38.8% decrease.
The month of January 2020 demonstrated the largest increase in driving, from `r jan_2018` in January 2018 to `r jan_2020` in January 2020. This was a 6.4% increase.

If we normalize our results to per 100000 capita:

The month of April 2020 demonstrated the largest decrease in miles per capita, from `r miles_per_capita_apr_2018` in April 2018 to `r miles_per_capita_apr_2020` in April 2020. This was a 39.3% decrease.
The month of January 2020 demonstrated the largest increase in miles per capita, from `r miles_per_capita_jan_2018` in January 2018 to `r miles_per_capita_jan_2020`. This was a 5.5% increase.
...
The fixed effect model in part 6 shows that the slope of the variable "miles driven per capita" is 0.00093498 with a statistically significantly p-value. If the number of miles driven per capita were to increase by as much as `r diff_per_capita_jan`, the difference between January 2018 and January 2020, the model predicts that the number of traffic fatalities would increase by `r change_100000_jan` per 100000 capita.
...
The fixed effect model in part 6 shows that the slope of the variable "miles driven per capita" is 0.00093498 with a statistically significantly p-value. If the number of miles driven per capita were to decrease by as much as `r diff_per_capita_apr`, the difference between April 2018 and April 2020, the model predicts that the number of traffic fatalities would decrease by `r change_100000_jan` per 100000 capita.
...
Serial Correlation: If present, serial correlation does not cause the Ordinary Least Squares (OLS) estimators to be biased, but it can cause them to be inefficient. In other words, they no longer have the smallest possible variance among the class of linear unbiased estimators.

Standard Errors: The standard errors are likely to be biased, leading to incorrect statistical inferences (e.g., t-tests and confidence intervals).

Serial Correlation Test:

```{r}
library(plm)
pbgtest(fixed_effects)

```
There is strong evidence of serial correlation in the idiosyncratic errors. 
...
```
The Breusch-Pagan test result  indicates a highly significant result (with a p-value close to zero). This suggests strong evidence against the null hypothesis of homoskedasticity.
