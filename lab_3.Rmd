...
my_panel <- data %>%
  mutate(speed_limit = ifelse(sl55 >=0.5, "sl55", 
                     ifelse(sl65 >=0.5, "sl65", 
                     ifelse(sl70 >=0.5, "sl70", 
                     ifelse(sl75 >=0.5, "sl75", 
                     ifelse(slnone >=0.5, "slnone", "None")))))) %>% 
...
for (i in 1:nrow(my_panel)) {
  result_vector[i] <- year_names[my_panel[i,26:50] != 0]
}
# Add the new column to the dataframe
my_panel$year_of_observation <- as.factor(result_vector)
...
my_panel <- my_panel %>%
  mutate(blood_alc_lmt = ifelse(bac10 >=0.5, "bac10", 
                       ifelse(bac08 >=0.5, "bac08", "None"))) %>% 
  select(-bac10, -bac08, -sbprim, -sbsecon) %>% 
  mutate(blood_alc_lmt = as.factor(blood_alc_lmt),
         seatbelt = as.factor(seatbelt),
         zerotol = as.logical(ifelse(zerotol >=0.5, TRUE, FALSE)),
         gdl = as.logical(ifelse(gdl >=0.5, TRUE, FALSE)),
         perse = as.logical(ifelse(perse >=0.5, TRUE, FALSE)))
```

```{r rename columns}
names(my_panel) <- c("year", "state", "seat_belt", "min_age", "zero_tolerance",
...
Fatality is given as $total\_fatal$, total fatalities at this year and state and additional $night\_fatal$ and $weekend\_fatal$ variables giving tempral details. Data normalized by to 100,000 people and to 100 milion miles driven is also presented in two groups of three variable. 
