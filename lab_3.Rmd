...
(speed_lim_plot | no_proof_plot)/(seat_belt_plot|alc_plot)
```
...
driven_plot <- my_panel %>%
  filter(state_abbr %in% state_slice ) %>%
  ggplot(aes(x = miles_per_capita, y = total_fatal_per_pop)) +
  geom_line() +
  facet_wrap(~ state_abbr, nrow = 3) +
  labs(x = "Miles driven",  y = "Fatalities/100k people") +
  theme(legend.position = "none", strip.text = element_text(size = 6))  + scale_x_discrete(labels = c("0" = "N", "1" = "Y", "2" = "Mbe"))
...
(unempl_plot | young_plot)/(driven_plot|gradual_plot)
```
